...
classifier = tf.estimator.Estimator(
    model_dir="tmp/cnn_model2", model_fn=cnn_model_fn)
...
def tf_predict(classifier, image):
    ...
    pred_input_fn = tf.estimator.inputs.numpy_input_fn(
		x={"x":processed_array}, shuffle=False)
...
def put_splitted_text_in_blackboard(blackboard, splitted_text):
	...
	for text in splitted_text:
		cv2.putText(blackboard, text, (4, y),
			...
			if cv2.contourArea(contour) > 10000:
				...
				if w1 > h1:
					save_img = cv2.copyMakeBorder(
						save_img, int((w1-h1)/2) , int((w1-h1)/2) , 0, 0,
						cv2.BORDER_CONSTANT, (0, 0, 0))
				elif h1 > w1:
					save_img = cv2.copyMakeBorder(
						save_img, 0, 0, int((h1-w1)/2) , int((h1-w1)/2) ,
						cv2.BORDER_CONSTANT, (0, 0, 0))
